...
function getFavourites() {
  //returns an array of JSON objects marked as favorites from localstorage
...
const Favourites = () => {
  ...
  const getLocationWeather = async (location) => {
    let url = `https://api.openweathermap.org/data/2.5/forecast?q=${location}&appid=${API_KEY}&units=${localStorage.getItem(
      "settings"
    )}`;
  ...
  function getLocationMap() {
    // creates a new favorite button for each JSON object in the favouritesArray

    if (favouritesArray) {
      return favouritesArray.favourite.map((favourite) => (
  ...
  function clear() {
    //removes all favourites locations
